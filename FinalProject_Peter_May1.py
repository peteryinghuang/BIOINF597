...
def generate_molecule(model, device, vocab, inv_vocab, num_samples, max_length=29):
    model.eval()
    smiles = []
    with torch.no_grad():
...
train_smiles_set = set(validate_data['smiles'].tolist())  # Use set for faster lookup
...
# Set a random seed as the value of 42
seed = 43
...
# Function to draw a set of molecules given their SMILES strings
def draw_molecules(smiles_list, mols_per_row=4):    
    """
    Draw a grid of molecules.

    :param smiles_list: list of SMILES strings of the valid molecules.
    :param mols_per_row: number of molecules to draw per row. Default is 4.
    :return: An IPython.display.Image object that contains the rendered image.
    """
    # Convert SMILES strings to RDKit Mol objects
    mols = [Chem.MolFromSmiles(smi) for smi in smiles_list if smi]  # filter out None values

    # Use RDKit to draw the molecule grid
    img = Draw.MolsToGridImage(mols, molsPerRow=mols_per_row, subImgSize=(200, 200), useSVG=True)
    return img

# Assuming `valid_smiles` are the valid SMILES strings from your code
img = draw_molecules(valid_smiles)
display(img)
